...
const page = () => {
  const galleryImages = [
  { id: 1, category: 'Auction', src: '/assets/footerimg.jpg' }, // Using footerimg as placeholder per request
  ...
  // Extra data for other tabs to show functionality
...
const categories = ['Match Day', 'Trophy Launch', 'Auction'];
  ...
  return (
     ...
     <Navbar />
      ...
      {/* 2. BACKGROUND (Pink to Blue Gradient) */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-8 flex flex-col items-center">
            ...
            <button
              ...
              className={`
                ...
                ${activeFilter === category 
                  ? 'bg-[#3d3db5] text-white shadow-lg border border-[#3d3db5] scale-105' // Active State (Solid Blue)
                  : 'bg-transparent text-white border border-white hover:bg-white/10' // Inactive State (Outline)
                }
              `}
            >
              {category}
        ...
        {/* 4. IMAGE GRID */}
        {/* Mobile: 1 col (centered) | Tablet: 2 cols | Desktop: 4 cols */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full animate-fadeIn">
          ...
          {/* Fallback if no images found for a category */}
